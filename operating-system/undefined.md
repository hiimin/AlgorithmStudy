...
###  데커\(Dekker\) 알고리즘
 ...
 두 프로세스\(또는 스레드\)가 하나의 자원을 혼란 없이 공유할 수 있게 한다. 두 프로세스가 동시에 임계 영역에 들어가려고 할 때 하나만 들어가도록 한다. 한 프로세스가 이미 임계영역에 있다면, 다른 프로세스는 전 프로세스가 끝나기를 기다려야 한다.

f0과 f1 두 개의 플래그로 프로세스 중 누가 임계 영역에 진입할 것인지를 나타내고, turn 변수로 누가 임계영역에 들어갈 차례인지 나타낸다.

```java
while(true) {
    f0 = true; // 프로세스 0가 임계 구역 진입 시도
    while(f1) { // 프로세스 1가 현재 임계 구역에 있는지 확인
        if(turn == 1) { // 1가 임계 구역 사용 중이면
            f0 = false; // 프로세스 0 진입 취소
            while(turn == 0); // turn이 1에서 변경될 때까지 대기
            f0 = true; // 1 turn이 끝나면 다시 진입 시도
        }
    }
}
...
turn = 1; // 임계 구역 사용 끝나면 turn을 넘김
f0 = false; // flag 값을 false로 바꿔 임계 구역 사용 완료를 알림
```

###  피터슨\(Peterson\) 알고리즘

 데커 알고리즘과 유사하지만, 상대방 프로세스/스레드에 진입 기회를 양보하는 것에 차이가 있음

```java
while(true) {
    flagI = true; // 프로세스 i가 임계 구역 진입 시도
    turn = J; // 다른 프로세스에게 진입 기회 양보
    while(flagJ && turn == J) { // 다른 프로세스가 진입 시도하면 대기
    }
}

// ------- 임계 구역 ---------

flagI = false; // flag 값을 false로 바꿔 임계 구역 사용 완료를 알림
```

###  제과점\(bakery\) 알고리즘

 여러 프로세스/스레드에 대한 처리가 가능한 알고리즘. 빵집에서 손님들이 도착하는 순서대로\(FIFO\) 번호표를 나눠주고 차례로 빵을 파는데에서 유. 가장 작은 수의 번호표를 가지고 있는 프로세스가 임계영역에 진입한다.
...
while(true) {
    ...
    for(j = 0; j < n; j++) { // 모든 프로세스 번호표 비교
        ...
        // 프로세스 j가 번호표를 갖고 있고, j의 번호표가 i의 번호표보다
